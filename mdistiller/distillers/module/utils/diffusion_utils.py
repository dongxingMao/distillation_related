...
def p_sample(model, x, y_t, t, alphas, one_minus_alphas_bar_sqrt, stochastic=False, fq_x=None):
    """
    ...
    eps_theta = model(x, y_t, t ).to(device).detach()
...
def p_sample_t_1to0(model, x, y_t, one_minus_alphas_bar_sqrt, fq_x=None):
    device = next(model.parameters()).device
...
def p_sample_loop(model, x, n_steps, alphas, one_minus_alphas_bar_sqrt,
    ...
    for t in reversed(range(1, n_steps)):
        y_t = y_t
        y_t = p_sample(model, x, y_t, t, alphas, one_minus_alphas_bar_sqrt, stochastic=stochastic, fq_x=fq_x)  # y_{t-1}
        if only_last_sample:
            num_t += 1
        else:
            # y_p_seq.append(y_t)
    ...
    if only_last_sample:
        ...
        y_0 = p_sample_t_1to0(model, x, y_t, one_minus_alphas_bar_sqrt, fq_x=fq_x)
        return y_0
    else:
        # assert len(y_p_seq) == n_steps
        y_0 = p_sample_t_1to0(model, x, y_p_seq[:, :, 1],  one_minus_alphas_bar_sqrt, fq_x=fq_x)
        # y_p_seq.append(y_0)
        y_p_seq[:, :, 0] = y_0
        return y_p_seq
...
def ddim_sample_loop(model, x_embed, timesteps, y_dim, ddim_alphas, ddim_alphas_prev, ddim_sigmas, stochastic=True, fq_x=None):
    device = next(model.parameters()).device
    batch_size = x_embed.shape[0]

    if fq_x is None:
        y_t = stochastic * torch.randn_like(torch.zeros([batch_size, x_embed.shape[1]])).to(device)
    else:
        y_t = stochastic * torch.randn_like(torch.zeros([batch_size, x_embed.shape[1]])).to(device) + fq_x
    ...
    for i, step in enumerate(time_range):
        ...
        import pdb
        # pdb.set_trace()
        y_t, pred_y0 = ddim_sample_step(model, x_embed, y_t, t, index, ddim_alphas,
...
def ddim_sample_step(model, x_embed, y_t, t, index, ddim_alphas, ddim_alphas_prev, ddim_sigmas, fq_x=None):
    batch_size = x_embed.shape[0]
    device = next(model.parameters()).device
    e_t = model(x_embed, y_t, t).to(device).detach()

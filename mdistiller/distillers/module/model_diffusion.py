...
import pdb
# from utils ResNet_for_32 as resnet_s
# import utils.ResNet_for_224 as resnet_l
...
class ConditionalModel(nn.Module):
    def __init__(self, n_steps, y_dim=256, fp_dim=128, feature_dim=None, guidance=True):
        ...
        if self.guidance:
            ...
            self.lin1 = ConditionalLinear(feature_dim, feature_dim, n_steps)
...
class Diffusion(nn.Module):
    ...
    def __init__(self, num_timesteps=1000, n_class=10, fp_dim=512, device='cuda', beta_schedule='cosine',
                 feature_dim=2048, ddim_num_steps=10):
        super().__init__()
        self.device = device
        ...
        self.model = ConditionalModel(self.num_timesteps, y_dim=self.n_class, fp_dim=fp_dim,
                                      feature_dim=feature_dim, guidance=False).to(self.device)
    ...
    def forward_t(self, y_0_batch, x_batch, t, fp_x=None, fq_x=None):
        # x_batch 是 student feature  ； y0 batch 是 teacher feature
        ...
        x_embed_batch = x_batch
        
        output = self.model(x_embed_batch, y_t_batch, t)
    ...
    def reverse(self, images, only_last_sample=True, stochastic=True, fp_x=None, fq_x=None):
        ...
        with torch.no_grad():
            ...
            label_t_0 = p_sample_loop(self.model, images,
    ...
    def reverse_ddim(self, x_batch, stochastic=True, fp_x=None, fq_x=None):
        ...
        with torch.no_grad():
            ...
            # x_embed_batch = self.diffusion_encoder(x_batch)
            # pdb.set_trace()
            label_t_0 = ddim_sample_loop(self.model, x_batch, self.ddim_timesteps, self.n_class, self.ddim_alphas,

...
def validate(val_loader, distiller, diffuser=True):
    ...
    with torch.no_grad():
        ...
        for idx, (image, target) in enumerate(val_loader):
            ...
            if diffuser is not None:
                # if isinstance(distiller, torch.nn.DataParallel):
                #     distiller = 
                _, feats_student = distiller.module.student(image)
                
                label_t_0 = distiller.module.diffusion_model.reverse_ddim(feats_student["pooled_feat"], stochastic=False, fq_x=None)
                output = distiller.module.student.fc(label_t_0)
